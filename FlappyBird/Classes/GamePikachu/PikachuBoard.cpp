...
PikachuBoard::PikachuBoard(int n_rows, int n_columns, int n_types, std::vector<int> count) :
...
int PikachuBoard::getNColumns()
{
	return n_columns;
}

void PikachuBoard::addPokemon(int x, int y, int type)
{
	_pokemons[x][y] = type;
}

int PikachuBoard::getPokemon(int x, int y)
{
...
bool PikachuBoard::canConnect(int _x, int _y, int x, int y)
{
	auto path = findPath(_x, _y, x, y);
	return path.size() >= 2 && path.size() <= 4;
}

std::vector<std::pair<int, int>> PikachuBoard::findPath(int _x, int _y, int x, int y)
{
	//INIT Graph
